...
def newtonRaphsonM(f, df, d2f, x, imax=100, tol=1e-8):
    cumple = False
    print('{:^10s} {:^10s} {:^10s} {:^10s}'.format('x', 'f(x)', 'df(x)', 'd2f(x)'))
    
    k = 0
    while (not cumple and k < imax):
        # Update x using the Modified Newton-Raphson formula
    ...
    if k < imax:
        ...
        raise ValueError('The function does not converge')
...
def main():
    ...
    plt.text(raiz, f(raiz), ' Root ' + str(raiz), color='red')
    plt.grid()
